...
# use when running the code and disable when knitting the pdf 
setwd("./src/objectives/")
...
Bike-sharing is a popular service that allows people to pick up a bike from one dock, use it and return it to the same or another docking station all for a small fee, sometimes for free. As opposed to transport like cars, bikes offer an inexpensive, eco-friendly option to urban people by promoting less pollution and dealing with less traffic. As such, bike-sharing programs have become very popular and in turn provide a wealth of data on the mobility in a city.

Data gathered by the rental companies includes things like temperature, count of users, humidity and more. The explicit collection of data has the potential to assist in understanding the highs and lows of bike demand which could influence; when to offer a sale, which areas require redistribution, and in general when the best time to offer rentals is. Future data to inquire about and include could be the demographics of the city, the style of current marketing and how often redistribution is needed.
...
```{r arma-seven-days-forecast-prediction-limits, message=FALSE, echo=FALSE}

knitr::kable(head(data.frame(forecast.2012.june$f, forecast.2012.june$ll, forecast.2012.june$ul), 30), "latex", booktabs = T)
...
```{r arma-one-day-forecast-prediction-limits, message=FALSE, echo=FALSE}

knitr::kable(data.frame(forecast.2012.june$f, forecast.2012.june$ll, forecast.2012.june$ul), "latex", booktabs = T)
...
```{r var-seven-days-forecast-prediction-limits, message=FALSE, echo=FALSE}

knitr::kable(head(data.frame(forecasts.var.seven.days$fcst$y1), 30), "latex", booktabs = T)
...
```{r var-one-day-forecast-prediction-limits, message=FALSE, echo=FALSE}

knitr::kable(data.frame(forecasts.var.one.day$fcst$y1), "latex", booktabs = T)
...
Here, we include the ensemble model error to compare. Our ensemble is built out of a rounded average between VAR and MLP. It turns out that for the 7-day, “long-term”, forecast the MLP performs best. Followed by the ensemble, var and then finally arma. Var likely performs better because it considers other data that happened at the same time compared to a strict ARMA(25,1). Similarly, the MLP model may perform well because of its ability to use multiple layers to understand how all the time series work together, with lag in mind. 
...
VAR was the preferred method for short term forecasts.  But note that the first six observations all have 1s as these were imputed.  VAR had a tendency to predict negative values early on, which wouldn’t make any sense given that our hourly counts should never be zero or less.  And it turns out that there were not any observations with fewer than one.  
...
```{r arma-logged-count-forecasts-seven-days-one, results='hide', message=FALSE, include=FALSE, echo=FALSE}

# 7 day forecast

df <- train.2012.june

df$lcount <- log(df$count)

# aic5.wge(df$lcount, p=20:26, q=0:2, type = "aic") # AIC picks ARMA (25,2)

est.2012.june = est.arma.wge(df$lcount, p=25, q=2)
est.2012.june$phi
est.2012.june$theta
est.2012.june$avar
...
forecast.2012.june = fore.aruma.wge(df$lcount, phi = est.2012.june$phi, theta = est.2012.june$theta, n.ahead = no_ahead, lastn = TRUE, limits = FALSE, plot = FALSE)


ase.arma.forecasts.seven.days.logged = mean((df$count[(len_of_obs - no_ahead + 1): len_of_obs] - exp(forecast.2012.june$f))^2)
ase.arma.forecasts.seven.days.logged


rmse.arma.forecasts.seven.days.logged = RMSE(y_true = df$count[(len_of_obs - no_ahead + 1): len_of_obs], y_pred = exp(forecast.2012.june$f))
rmse.arma.forecasts.seven.days.logged


```

```{r arma-logged-count-forecasts-seven-days-two, results='hide', message=FALSE, include=FALSE, echo=FALSE}

forecasts.arma.25.2.seven.days.logged = round(exp(forecast.2012.june$f))
# set point forecast to 1 when less than 0
forecasts.arma.25.2.seven.days.logged[forecasts.arma.25.2.seven.days.logged < 0] <- 1

```


```{r arma-logged-count-forecasts-seven-days-three, message=FALSE, echo=FALSE, warning=FALSE}

df2 <- data.frame(real = df$count,
          pred = c(rep(NA,length(df$datetime)-length(data.frame(forecasts.arma.25.2.seven.days.logged)[,1])),data.frame(forecasts.arma.25.2.seven.days.logged)[,1]),
          time = df$datetime)

plot.arma.seven.days.logged <- ggplot(df2, aes(time, real, group = 1 )) +
  geom_line() + 
  geom_line(aes(time, pred, group = 1), color = "red")+
  ggtitle("7 Day Forecast (ARMA)") +
  labs(x = "Day", y = "Count") + 
...
- **ASE** `r round(ase.arma.forecasts.seven.days.logged, 3)`

- **RMSE** `r round(rmse.arma.forecasts.seven.days.logged, 3)`

\pagebreak

### 1 Day Forecast

```{r arma-logged-count-forecasts-one-day-one, results='hide', message=FALSE, include=FALSE, echo=FALSE}
...
ase.arma.forecasts.one.day.logged = mean((df$count[(len_of_obs - no_ahead + 1): len_of_obs] - exp(forecast.2012.june$f))^2)
ase.arma.forecasts.one.day.logged

rmse.arma.forecasts.one.day.logged = RMSE(y_true = df$count[(len_of_obs - no_ahead + 1): len_of_obs], y_pred = exp(forecast.2012.june$f))
rmse.arma.forecasts.one.day.logged


```

```{r arma-logged-count-forecasts-one-day-two, results='hide', message=FALSE, include=FALSE, echo=FALSE}

forecasts.arma.25.2.one.day.logged = round(exp(forecast.2012.june$f))
forecasts.arma.25.2.one.day.logged[forecasts.arma.25.2.one.day.logged < 0] <- 1

```



```{r arma-logged-count-forecasts-one-day-three, message=FALSE, echo=FALSE, warning=FALSE}

df2 <- data.frame(real = df$count,
          pred = c(rep(NA,length(df$count)-length(data.frame(forecasts.arma.25.2.one.day.logged)[,1])),data.frame(forecasts.arma.25.2.one.day.logged)[,1]),
          time = df$datetime)

plot.arma.one.day.logged <- ggplot(df2, aes(time, real, group = 1 )) +
  geom_line() + 
  geom_line(aes(time, pred, group = 1), color = "red")+
  ggtitle("1 Day Forecast (ARMA)") +
  labs(x = "Day", y = "Count") + 
...
plot.arma.one.day.logged

```

- **ASE** `r round(ase.arma.forecasts.one.day.logged, 3)`

- **RMSE** `r round(rmse.arma.forecasts.one.day.logged, 3)`
