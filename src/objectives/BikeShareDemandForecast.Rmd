...
```{r differncing-d1-june-2012, results='hide', message=FALSE, include=FALSE, echo=FALSE}
...
```{r roll-win-ase-wge-function-output, results='hide', message=FALSE, include=FALSE, echo=FALSE}
...
1.
	i) The primary response variable, count, for the Bike Sales Demand data looks skewed. What kind of transformation is ideal for this time series realization? 
	ii) Find and compare the ASEs for both the ARMA and the VAR models as they apply to the bike data. Which model performs better on the long-term forecast(7 days) and the short-term forecast(1 day)? (Hint: This is hourly data)
...
forecast.2012.june = fore.aruma.wge(lcount, phi = est.2012.june$phi, theta = est.2012.june$theta, n.ahead = no_ahead, lastn = TRUE, limits = FALSE, plot = FALSE)

# previously commented section
plot(lcount[1:len_of_obs], type = "l", xlab = "Time", ylab = "Logged Count", main = "7 Day Forecast (ARMA)")
lines(seq(289,456), forecast.2012.june$f, col = "red")
# previously commented section

ase.arma.forecasts.seven.days = mean((lcount[(len_of_obs - no_ahead + 1): len_of_obs] - forecast.2012.june$f)^2)
#ase.arma.forecasts.seven.days
scale_ase.arma.forecasts.seven.days = exp(ase.arma.forecasts.seven.days)
scale_ase.arma.forecasts.seven.days

rmse.arma.forecasts.seven.days = RMSE(y_true = lcount[(len_of_obs - no_ahead + 1): len_of_obs], y_pred = forecast.2012.june$f)
#rmse.arma.forecasts.seven.days
scale_rmse.arma.forecasts.seven.days = exp(rmse.arma.forecasts.seven.days)
scale_rmse.arma.forecasts.seven.days
...
# previously commented section
predictions = round(forecast.2012.june$f)
actuals = lcount[(len_of_obs - no_ahead + 1): len_of_obs]
...
# previously commented section
plot(lcount[1:len_of_obs], type = "l", xlab = "Time", ylab = "Logged Count", main = "1 Day Forecast (ARMA)")
lines(seq(433,456), forecast.2012.june$f, col = "red")
# previously commented section

ase.arma.forecasts.one.day = mean((lcount[(len_of_obs - no_ahead + 1): len_of_obs] - forecast.2012.june$f)^2)
# ase.arma.forecasts.one.day
scale_ase.arma.forecasts.one.day = exp(ase.arma.forecasts.one.day)
scale_ase.arma.forecasts.one.day

rmse.arma.forecasts.one.day = RMSE(y_true = lcount[(len_of_obs - no_ahead + 1): len_of_obs], y_pred = forecast.2012.june$f)
# rmse.arma.forecasts.one.day
scale_rmse.arma.forecasts.one.day=exp(rmse.arma.forecasts.one.day)
scale_rmse.arma.forecasts.one.day
...
df2 <- data.frame(real = lcount,
          pred = c(rep(NA,length(lcount)-length(data.frame(forecasts.arma.25.1.one.day)[,1])),data.frame(forecasts.arma.25.1.one.day)[,1]),
          time = df$datetime)
...
Can additional data from previous month(s) aid in lowering the ASE and RMSE?  


Including data from May, 2012 seemed to indicate that you could, albeit marginally.  However, attempting to use observations from more than 2 months out was detrimental to the model's performance.
...
And the RMSE stood at `r round(rmse.mlp.forecasts.one.day.2, 3)`.
