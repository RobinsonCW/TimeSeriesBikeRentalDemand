...
Fall appears to have more rentals on average than the other seasons.
...
The same was true for the day falling on a holiday, it appeared to have little affect on the counts.
...
The plot below shows a strong correlation between casual, registered and total (count) users.  We will use count as the primary response variable for our modeling, and discard the other response variables.
...
ccf(train.2012.june$temp, train.2012.june$count)
...
ccf(train.2012.june$humidity, train.2012.june$count, lag.max = 25)
...
ccf(train.2012.june$weather, train.2012.june$count, lag.max = 25)
...
ccf(train.2012.june$hour, train.2012.june$count, lag.max = 25)
...
# xreg.lags = c(1,1,1,24),   << For some reason, the code will not run with these in
...
Use all monthly data leading up to June 2012 
